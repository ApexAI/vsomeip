...
policy::print() const {
    ...
    for (auto its_credential : credentials_) {
        auto its_uid_interval = its_credential.first;
        if (static_cast<uint32_t>(its_uid_interval.lower()) == std::numeric_limits<uint32_t>::max()) {
        ...
        for (auto its_gid_interval : its_credential.second) {
            if (static_cast<uint32_t>(its_gid_interval.lower()) == std::numeric_limits<uint32_t>::max()) {
